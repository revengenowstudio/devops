...
from config_data_source import (
...
def main():
    if should_args_exist(short_arg="--help", long_arg="-h") or len(sys.argv) == 1:
    ...
    for name, version in zip(
        (Log.version_start, Log.version_end), (config.version_start, config.version_end)
    ):
        try:
    ...
    # 拉取归档文件内容
    archive_document_collector = ArchiveDocumentCollector(config.repository_token)
    ...
    for index, archived_issues_info in enumerate(config.archived_issues_info):
        ...
        try:
            ...
            print(
                ErrorMessage.collect_document_error.format(index=index_, exc=str(exc))
            )
    print(Log.collect_document_success_number.format(number=len(document_content_list)))
    ...
    for document_content in document_content_list:
        ...
        print(
            Log.match_much_archive_content.format(
